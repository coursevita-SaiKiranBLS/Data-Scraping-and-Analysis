...
def create_collections_with_data(collection_names):
    for collection_name in collection_names:
        create_collection_with_example_data(collection_name)

def create_collection_with_example_data(collection_name):
    ...
    # Add example documents (job titles) to the collection
    for job_title in example_job_titles:
        doc_ref = collection_ref.document()  # Firestore will generate a unique document ID
        doc_ref.set({'title': job_title})
        print(f'Added document with job title: {job_title} to collection: {collection_name}')

# List of job categories (collection names)
job_categories = [
...
# Create collections for each job category
create_collections_with_data(job_categories)
