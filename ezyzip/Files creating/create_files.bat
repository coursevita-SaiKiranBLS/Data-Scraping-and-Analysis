...
for %%F in %file_names% do (
    echo. > "%%F"
    echo Created file: %%F
    echo Appending code to %%F
    (
        type "%%F"  
        echo.       
        echo.       
        type "C:\Users\bayya\OneDrive\Desktop\web scraper\Files creating\app.py"  
    ) > "%%F.tmp"  
    move /y "%%F.tmp" "%%F"  
)

echo All files created and updated in "%folder_path%".
pause
